...
Reasoner Agent 深度分析 + 持仓健康度评估 + 闭环迭代因子进化引擎，TqSdk 拉取数据写入本地 DuckDB，纯 Python 做计算，LLM 做推理，每日自动扫描 86 个主力合约（筛选非僵尸品种），信号输出附带仓位建议和止损价位。
...
- **种子因子池**: 研报知识注入 + 预置种子因子
- **失败经验库**: 从失败中学习，避免重复错误
- **全品种扫描**: 86 个主力合约，筛选非僵尸品种（持仓量≥10000）
- **VGRSI 可见图因子**: 基于论文 arXiv:2605.01300，A0/A1 两种聚合模式
- **Walk-Forward 验证**: 30天优化+7天测试滚动验证，防止参数过拟合
...
| VisibilityGraphOperator | `visibility_graph_operator.py` | 可见图算子管理器 |
| MultiTimeframeVGRSIFactor | `visibility_graph.py` | 多周期 VGRSI 一致性因子 |
| VolatilityAnchor | `volatility_anchor.py` | 波动幅度止损锚点 |
