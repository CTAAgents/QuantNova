...
满足以下**任一**条件即触发 LLM 推理（OR 逻辑，可在 `config.json` 中切换为 AND）：
...
**配置项**：`config.json` → `scanner.signal_filter.filter_mode`
- `"or"`（默认）：任一条件满足即触发，信号更多但覆盖面广
- `"and"`：所有条件同时满足才触发，信号更少但质量更高

信号强度根据满足条件数量自动判定：4/4=STRONG，2-3/4=MEDIUM，1/4=WEAK。
