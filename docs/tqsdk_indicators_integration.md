...
> 版本：v1.1 | 创建日期：2026-06-17 | 更新：2026-06-18
> 状态：进行中
...
| 类别 | 数量 | 说明 |
|------|------|------|
| **IndicatorEngine（本地计算）** | 35+ | 基于 pandas 的技术指标计算 |
| **sync_indicators.py（预计算）** | 80+ | 预计算并存储到 DuckDB |
| **TqSdk ta 模块（参考）** | 70 | TqSdk 提供的指标集（参考实现） |

### 1.2 指标来源

1. **IndicatorEngine**：实时计算 35+ 个技术指标，用于信号扫描
2. **sync_indicators.py**：预计算 80+ 个指标，存储到 DuckDB 用于快速查询
3. **TqSdk ta 模块**：参考实现，用于验证本地计算结果

---

## 二、IndicatorEngine 指标列表（35+ 个）
