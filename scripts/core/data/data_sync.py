...
class DataSyncManager:
    ...
    def sync_symbols(self) -> dict[str, Any]:
        ...
        if not symbols:
            return {"success": False, "error": "无法获取品种列表", "count": 0}
    ...
    def sync_quotes(self, symbols: list[str] = None, min_oi: float = 0) -> dict[str, Any]:
        ...
        if not symbols:
            return {"success": False, "error": "没有品种需要同步", "count": 0}
        ...
        if not quotes:
            return {"success": False, "error": "无法获取行情数据", "count": 0}
        ...
        return {"success": True, "count": synced_count, "active_count": active_count, "min_oi": min_oi}
    ...
    def sync_klines(
        self, symbols: list[str] = None, days: int = 120, timeframe: str = "daily", force: bool = False
    ) -> dict[str, Any]:
        """
        同步K线数据
            ...
            try:
                ...
                try:
                    ...
                    indicator_cols = [
                        ...
                        if col not in ["date", "open", "high", "low", "close", "volume", "open_interest"]
                    ]
    ...
    def get_kline(
        self, symbol: str, days: int = 120, timeframe: str = "daily", allow_tqsdk_fallback: bool = True
    ) -> pd.DataFrame | None:
        """
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        Returns:
            统计信息字典
        """
        return {"sqlite": self.sqlite.get_statistics(), "duckdb": self.duckdb.get_statistics()}
...
def main():
    ...
    parser.add_argument("action", choices=["sync", "stats", "symbols", "quotes", "klines"], help="操作类型")
