...
class DuckDBStore:
    ...
    def get_klines(
        self, symbol: str, timeframe: str = "daily", start_date: str = None, end_date: str = None, limit: int = 1000
    ) -> pd.DataFrame:
        """获取 K 线数据"""
        if not self.conn:
            return pd.DataFrame()
    ...
    def get_indicators(
        self, symbol: str, indicator_name: str = None, start_date: str = None, end_date: str = None
    ) -> pd.DataFrame:
        """获取技术指标"""
        if not self.conn:
            return pd.DataFrame()
    ...
    def analyze_performance(self, symbol: str = None, days: int = 30) -> dict[str, Any]:
        ...
        return {"symbol": symbol, "analysis_period_days": days, "note": "交易性能分析需要从 SQLite 获取数据"}
