...
DEFAULT_ANCHORS = [
    # ===== 动量维度 =====
    {
        ...
        "factor_seeds": [
            ...
            {
                "expression": "rsi_14 * (1 - trend_strength)",
                "name": "rsi_trend_adjusted",
    ...
    },
    {
        ...
        "dimension": "momentum",
        "title": "MACD柱状图动量",
        "core_logic": "MACD柱状图由负转正/由正转负，是动量反转的领先信号。柱状图斜率加速表示趋势增强。",
        "factor_seeds": [
            {"expression": "macd_hist", "name": "macd_hist_raw", "description": "MACD柱状图原始值"},
            {
                "expression": "macd_hist - macd_hist.shift(1)",
                "name": "macd_hist_accel",
    ...
    # ===== 趋势维度 =====
    {
        ...
        "factor_seeds": [
            ...
            {"expression": "er * r_squared", "name": "er_r2_composite", "description": "ER与R²乘积，双重确认趋势强度"},
        ],
    ...
    },
    {
        ...
        "dimension": "trend",
        "title": "Hurst指数趋势持续性",
        "core_logic": "H>0.5趋势持续，H<0.5均值回归，H≈0.5随机游走。Hurst指数是判断趋势持续性的数学基础。",
        "factor_seeds": [
            {"expression": "hurst", "name": "hurst_raw", "description": "Hurst指数原始值"},
            {
                "expression": "hurst - 0.5",
                "name": "hurst_deviation",
    ...
    },
    {
        ...
        "dimension": "trend",
        "title": "复合趋势强度",
        "core_logic": "综合ER、R²、TSI、ADX、Hurst等指标构建复合趋势强度评分。多指标共识比单一指标更可靠。",
        "factor_seeds": [
            {
                "expression": "trend_strength_composite",
    ...
    # ===== 波动率维度 =====
    {
        ...
        "title": "ATR突破",
        "core_logic": "价格突破ATR通道（如±2ATR），是趋势启动信号。ATR扩张伴随价格突破→新趋势启动；ATR收缩→盘整蓄势。",
        "factor_seeds": [
            {
                "expression": "(close - ma20) / atr_14",
                "name": "price_atr_ratio",
                "description": "价格偏离均线的ATR归一化值",
            },
            {
                "expression": "atr_14 / atr_14.shift(20) - 1",
                "name": "atr_expansion",
    ...
    },
    {
        ...
        "dimension": "volatility",
        "title": "布林带收窄",
        "core_logic": "布林带带宽收窄至历史低位，预示波动率即将爆发。带宽 = (上轨-下轨)/中轨，带宽<20%为收窄。",
        "factor_seeds": [
            {"expression": "(bb_upper - bb_lower) / bb_middle", "name": "bb_bandwidth", "description": "布林带带宽"},
            {
                "expression": "1 - (bb_upper - bb_lower) / (bb_upper - bb_lower).rolling(60).rank(pct=True)",
                "name": "bb_squeeze_score",
                "description": "布林带收窄评分，1=最窄",
            },
        ],
        "validation_rules": {"min_ic": 0.02, "min_ir": 0.3, "min_win_rate": 0.45, "max_drawdown": 15},
        "applicable_markets": ["all"],
    ...
    # ===== 成交量维度 =====
    {
        ...
        "dimension": "volume",
        "title": "OBV背离",
        "core_logic": "OBV(On-Balance Volume)与价格背离，预示趋势即将反转。价涨量跌→顶背离；价跌量涨→底背离。",
        "factor_seeds": [
            {"expression": "obv", "name": "obv_raw", "description": "OBV原始值"},
            {
                "expression": "obv / obv.rolling(20).mean() - 1",
                "name": "obv_deviation",
    ...
    # ===== 基差维度（FinClaw 新增） =====
    {
        ...
        "factor_seeds": [
            ...
            {
                "expression": "basis_rate * (1 - abs(basis_rate)/10)",
                "name": "basis_adjusted",
    ...
    # ===== 季节性维度（FinClaw 新增） =====
    {
        ...
        "factor_seeds": [
            {
                "expression": "current_month_signal",
                "name": "seasonal_signal",
                "description": "当前月份的历史平均涨跌幅",
            },
            {
                "expression": "current_month_pos_rate - 50",
                "name": "seasonal_bias",
...
class KnowledgeAnchorManager:
    ...
    def get_anchor(self, anchor_id: str) -> KnowledgeAnchor | None:
        """获取单个知识锚点"""
        try:
            conn = sqlite3.connect(self._db_path)
            cursor = conn.cursor()
    ...
    def get_anchors_by_dimension(self, dimension: str) -> list[KnowledgeAnchor]:
        ...
        try:
            conn = sqlite3.connect(self._db_path)
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM knowledge_anchors WHERE dimension = ? ORDER BY confidence DESC", (dimension,))
