...
class WalkForwardValidator:
    ...
    def validate(
        self, prices: pd.Series, factor_func: Callable, param_space: dict[str, Any], optimize_func: Callable
    ) -> WalkForwardResult:
        """
        执行 Walk-Forward 验证
        ...
        for i, start_idx in enumerate(window_starts):
            ...
            logger.info(
                f"窗口 {i + 1}: IS Sharpe={is_metrics['sharpe']:.3f}, "
    ...
    def _evaluate_window(self, prices: pd.Series, factor_func: Callable, params: dict[str, Any]) -> dict[str, float]:
        """
        评估单个窗口的表现
    ...
    def _check_pass_criteria(self, is_metrics: dict[str, float], oos_metrics: dict[str, float]) -> bool:
        """
        检查是否通过验证标准
