...
class StrategyHealthChecker:
    ...
    def __init__(
        self, sharpe_threshold: float = 0.5, drawdown_threshold: float = 15.0, win_rate_decline_threshold: float = 0.1
    ):
        """
        初始化健康度检查器
    ...
    def check(self, trades: list[dict[str, Any]], lookback: int = 50) -> dict[str, Any]:
        ...
        if not trades or len(trades) < 5:
            return {"health_score": 50, "status": "数据不足", "details": {}, "recommendation": "交易数据不足，无法评估"}
