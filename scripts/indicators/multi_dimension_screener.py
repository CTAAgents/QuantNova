            ...
            "dimensions": [
                {
                    ...
                    "top_indicators": dict(
                        sorted(d.indicator_scores.items(), key=lambda x: abs(x[1]), reverse=True)[:5]
                    ),
                }
...
class MultiDimensionScreener:
    ...
    def _score_dimension(self, dim_name: str, latest: pd.Series, df: pd.DataFrame) -> DimensionScore:
        ...
        if not scores:
            return DimensionScore(
                name=dim_name, weight=0, indicator_scores={}, composite=0, direction="NEUTRAL", confidence=0
            )
    ...
    @staticmethod
    def _normalize_bounded(val: float, neutral: float, lo: float, hi: float) -> float:
        """
        有界指标归一化：线性映射到 (-1, +1)。
    ...
    @staticmethod
    def _normalize_volume(ind_name: str, val: float, latest: pd.Series, df: pd.DataFrame) -> float:
        """
        成交量类指标归一化：检测量价背离。
        ...
        if ind_name == "obv" and "obv" in df.columns:
            ...
            price_trend = (df["close"].values[-1] - df["close"].values[-5]) / (abs(df["close"].values[-5]) + 1)

            # 同向 = +1, 背离 = -1, 不变 = 0
            if (obv_trend > 0.02 and price_trend > 0) or (obv_trend < -0.02 and price_trend < 0):
                return 0.7
            elif (obv_trend > 0.02 and price_trend < 0) or (obv_trend < -0.02 and price_trend > 0):
                return -0.7
            else:
                return 0.0
    ...
    @staticmethod
    def _normalize_channel(channel_type: str, ind_name: str, val: float, latest: pd.Series) -> float:
        """
...
def score_symbol(symbol: str, df: pd.DataFrame, weights: dict[str, float] | None = None) -> MultiDimensionResult:
    """便捷函数：对单个品种进行多维度评分"""
...
def batch_score(
    symbols_dfs: list[tuple[str, pd.DataFrame]], weights: dict[str, float] | None = None
