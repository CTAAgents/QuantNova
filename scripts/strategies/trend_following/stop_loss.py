...
class StopLossCalculator:
    ...
    def atr_stop(self, entry_price: float, atr: float, direction: str, multiplier: float = None) -> float:
        """
        ATR 倍数止损
    ...
    def trailing_stop(self, best_price: float, atr: float, direction: str, multiplier: float = 3.0) -> float:
        """
        移动止损（跟踪止损）
    ...
    def time_stop(
        self, entry_time: datetime, max_holding_days: int = 10, current_time: datetime = None
    ) -> dict[str, Any]:
        """
        时间止损
        ...
        return {"should_stop": False, "reason": f"持仓 {holding_days} 天，未超限", "holding_days": holding_days}
    ...
    def multi_condition_stop(
        ...
        stops["atr_stop"] = {"price": round(atr_stop, 2), "type": "ATR止损", "active": True}

        # 2. 移动止损（如果提供了最优价格）
        if best_price is not None:
            trail_stop = self.trailing_stop(best_price, atr, direction)
            # 移动止损只在盈利时激活
            if (direction == "LONG" and trail_stop > entry_price) or (
                direction == "SHORT" and trail_stop < entry_price
            ):
                stops["trailing_stop"] = {"price": round(trail_stop, 2), "type": "移动止损", "active": True}

        # 3. 波动率调整止损
        if current_vol is not None:
            vol_stop = self.volatility_adjusted_stop(entry_price, atr, direction, current_vol)
            stops["vol_adjusted_stop"] = {"price": round(vol_stop, 2), "type": "波动率调整止损", "active": True}
