...
class BeliefPropagationManager:
    ...
    def propagate_beliefs(
        ...
        logger.info(f"传播信念: {source_agent} -> {target_agent}, "
                    f"传播 {len(filtered_beliefs)} 个信念")
        
        return result
    
    def _find_matching_rule(self, source_agent: str, target_agent: str) -> Optional[PropagationRule]:
        """
        查找匹配的传播规则
...
class BeliefSynchronizer:
    ...
    def synchronize_beliefs(
        self,
        agent_beliefs: Dict[str, List[Any]]
    ) -> Dict[str, List[Any]]:
        """
        同步信念
