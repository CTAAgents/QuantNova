...
class UnifiedMemoryManager:
    ...
    def retrieve_experiences(
        self, context: dict[str, Any], top_k: int = 5, min_similarity: float = 0.5, time_decay: bool = True
    ) -> list[dict[str, Any]]:
        """
        检索相似经验
        ...
        # 1. 向量检索（粗筛）
        vector_matches = self.vector_store.search(query_vector=feature_vector, top_k=top_k * 3)
    ...
    def retrieve_experiences_multi_path(
        self, context: dict[str, Any], top_k: int = 5, min_similarity: float = 0.5
    ) -> list[dict[str, Any]]:
        """
        多路召回检索相似经验
        ...
        Returns:
            相似经验列表
        """
        return self.retriever.retrieve(context=context, top_k=top_k, min_similarity=min_similarity)
