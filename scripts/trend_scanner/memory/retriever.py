...
class SimilarityCalculator:
    ...
    def calculate(self, query: dict[str, Any], candidate: dict[str, Any]) -> float:
        """
        计算综合相似度
    ...
    def _structural_similarity(self, query: dict[str, Any], candidate: dict[str, Any]) -> float:
        """计算结构化相似度"""
        score = 0.0
        count = 0
...
class MultiPathRetriever:
    ...
    def retrieve(self, context: dict[str, Any], top_k: int = 5, min_similarity: float = 0.5) -> list[dict[str, Any]]:
        """
        多路召回相似经验
    ...
    def _vector_search(self, context: dict[str, Any], top_k: int) -> list[dict[str, Any]]:
        """向量相似度检索"""
        ...
        # 使用向量存储检索
        matches = self.memory.vector_store.search(query_vector=feature_vector, top_k=top_k)
    ...
    def _condition_search(self, context: dict[str, Any], top_k: int) -> list[dict[str, Any]]:
        """结构化条件检索"""
        ...
        # 按品种检索
        if symbol:
            experiences = self.memory.sqlite_store.get_recent_experiences(symbol=symbol, limit=top_k)
        else:
            experiences = self.memory.sqlite_store.get_recent_experiences(limit=top_k)

        # 过滤趋势阶段
        if trend_phase:
            experiences = [e for e in experiences if e.get("trend_phase") == trend_phase]

        return experiences

    def _time_search(self, context: dict[str, Any], top_k: int) -> list[dict[str, Any]]:
        """时间范围检索"""
        # 获取最近的经验
