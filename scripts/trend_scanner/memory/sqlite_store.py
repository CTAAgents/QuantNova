...
class SQLiteStore:
    ...
    def get_recent_experiences(self, symbol: str = None, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        if symbol:
            cursor.execute(
                "SELECT * FROM experiences WHERE symbol = ? ORDER BY timestamp DESC LIMIT ?",
                (symbol, limit)
            )
        else:
            cursor.execute(
    ...
    def get_active_rules(self, rule_type: str = None) -> List[Dict[str, Any]]:
        ...
        if rule_type:
            cursor.execute(
                "SELECT * FROM strategy_rules WHERE status = 'active' AND rule_type = ?",
                (rule_type,)
            )
        else:
    ...
    def get_recent_trades(self, symbol: str = None, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        if symbol:
            cursor.execute(
                "SELECT * FROM trade_journal WHERE symbol = ? ORDER BY timestamp DESC LIMIT ?",
                (symbol, limit)
            )
        else:
            cursor.execute(
    ...
    def get_active_patterns(self, pattern_type: str = None) -> List[Dict[str, Any]]:
        ...
        if pattern_type:
            cursor.execute(
                "SELECT * FROM pattern_library WHERE status = 'active' AND pattern_type = ?",
                (pattern_type,)
            )
        else:
    ...
    def _row_to_dict(self, row: sqlite3.Row) -> Dict[str, Any]:
        ...
        json_fields = [
            'feature_vector', 'context_snapshot', 'full_data',
            'applicable_phases', 'applicable_symbols',
            'brief_at_entry', 'brief_at_exit', 'lessons_learned', 'patterns_detected',
            'conditions', 'associated_rules'
        ]
