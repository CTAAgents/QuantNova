...
class OverfittingDetector:
    ...
    def __init__(
        self,
        sharpe_suspicious_threshold: float = 3.0,
    ...
    def sharpe_sanity_check(self, returns: List[float]) -> Dict[str, Any]:
        ...
        return {
            ...
            'reason': f'夏普比率 {sharpe:.2f} 超过 {self.sharpe_suspicious_threshold}，高度怀疑过拟合' if is_suspicious else '夏普比率在合理范围内'
        }
    ...
    def parameter_sensitivity(
        ...
        return {
            ...
            'is_overfit': is_overfit,
            'reason': f'参数敏感性 {avg_sensitivity:.2f} 超过阈值，策略对参数过于敏感' if is_overfit else '参数敏感性在可接受范围内'
        }
    
    def sample_split_test(
        self,
        returns: List[float],
