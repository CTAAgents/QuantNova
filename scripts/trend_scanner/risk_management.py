...
class RiskManager:
    ...
    def __init__(self, account_equity: float = 1_000_000, risk_per_trade: float = 0.01, atr_multiplier: float = 2.0):
    ...
    def calc_take_profit(self, entry: float, stop: float, direction: int, rr_ratio: float = 2.5) -> float:
        risk = abs(entry - stop)
    ...
    def calc_position(
        self, entry: float, stop: float, point_value: float = 10.0, margin_per_lot: float = 5000.0
    ...
    def adjust_by_signal(self, base_lots: int, signal_strength: str, adx_value: float) -> int:
    ...
    def calc_progressive_position(
        ...
        """
        # 计算总可开仓位
        total_lots, total_risk, total_margin = self.calc_position(entry, stop, point_value, margin_per_lot)
    ...
    def calc_stop_by_phase(self, entry: float, atr: float, direction: int, phase: str) -> float:
        """基于趋势阶段计算动态止损"""
    ...
    def adjust_position_by_phase(self, base_lots: int, phase: str, reliability_score: int) -> int:
        """基于趋势阶段和可靠性调整仓位"""
...
class ExitSignalGenerator:
    ...
    def detect_momentum_exhaustion(cls, df: pd.DataFrame, direction: int) -> dict:
        ...
        if ema20 > 0 and ema60 > 0:
            ...
            if (direction > 0 and ma_deviation > 5) or (direction < 0 and ma_deviation < -5):
                exhaustion_score += 15
    ...
    @classmethod
    def detect_trailing_stop(cls, df: pd.DataFrame, direction: int, entry_price: float, current_stop: float) -> dict:
        """
        跟踪止损检测
        ...
        return {"new_stop": round(new_stop, 2), "triggered": triggered, "reason": reason}
    ...
    def detect_reversal_signal(cls, df: pd.DataFrame, direction: int) -> dict:
        ...
        return {"signal": reversal_score >= 40, "strength": reversal_score, "reasons": reasons}

    @classmethod
    def generate_exit_signal(
        cls, df: pd.DataFrame, direction: int, entry_price: float, current_stop: float, target_profit: float = 0.05
    ) -> dict:
        """
        综合离场信号生成
