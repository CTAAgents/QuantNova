...
"""
    ...
    def _design_with_rules(
        ...
        design = StateSpaceDesign(
            design_id=f"state_design_rules",
            name="规则设计的状态空间",
            ...
            if metric_name in expected_metrics:
                ...
                if isinstance(expected_range, list) and len(expected_range) == 2:
                    ...
                    if min_val <= current_value <= max_val:
                        ...
                        status = 'warning'
                        suggestion = f'指标偏低，建议调整'
                    else:
                        status = 'warning'
                        suggestion = f'指标偏高，建议调整'
                else:
                    status = 'unknown'
                    suggestion = '无法判断'
...
class RLInterfaceDesigner:
    ...
    def design_interface(
        ...
        """
        # 设计状态空间
        state_design = self.state_designer.design_state_space(
            market_context, trading_objective, available_data
        )
        
        # 设计奖励函数
        reward_design = self.reward_designer.design_reward_function(
    ...
    def refine_interface(
        ...
        """
        # 收集诊断结果
        diagnostics = self.diagnostic_refiner.collect_diagnostics(
            training_metrics, expected_metrics
        )
        
        # 建议修正方案
        actions = self.diagnostic_refiner.suggest_refinements(
            diagnostics, current_design
        )
...
if __name__ == "__main__":
    ...
    # 设计接口
    interface = designer.design_interface(
