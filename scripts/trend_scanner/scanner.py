...
from .market_analysis import MarketStateClassifier, MultiIndicatorConsensus, TrendPhaseDetector
from .models import ScoringFeedback, TradeSignal
...
def _ensure_md_modules():
    ...
    try:
        ...
        # 数据库路径推导
        db_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "data", "market.db")
...
class TrendScanner:
    ...
    def __init__(
        ...
        ma_periods: dict[str, int] | None = None,
    ...
    def analyze(self, df: pd.DataFrame, symbol: str = "UNKNOWN", use_multi_dimension: bool = False) -> dict:
        """
        分析市场状态并生成交易信号
        ...
        # 3. 趋势阶段识别
        phase, phase_conf, reliability, breakdown, alerts, phase_evidence = TrendPhaseDetector.detect(df, market_state)
        ...
        if signal_str in ("BUY", "SELL") and atr > 0:
            ...
            lots, actual_risk, margin_used = risk_mgr.calc_progressive_position(
                entry, stop, phase, confirmation_level, self.point_value, self.margin_per_lot
            )
        ...
        if use_multi_dimension and _ensure_md_modules():
            try:
                ...
                if wide_df is not None and len(wide_df) > 0:
                    ...
                    # 注入维度明细到证据列表
                    dim_details = [f"{d.name}: {d.composite:+.3f} ({d.direction})" for d in md_result.dimensions]
                    all_evidence.append(
                    ...
                    if md_signal == "LONG" and signal_str == "BUY":
                        ...
                        all_evidence.append(f"[多维度确认] 五维度评分偏多({md_score:+.3f})，与多头信号一致")
                    elif md_signal == "SHORT" and signal_str == "SELL":
                        confidence = min(100, confidence + 5)
                        all_evidence.append(f"[多维度确认] 五维度评分偏空({md_score:+.3f})，与空头信号一致")
                    elif md_signal in ("LONG", "SHORT") and signal_str == "HOLD":
                        all_evidence.append(
                            f"[多维度提示] 五维度评分{md_signal}({md_score:+.3f})，但传统信号为HOLD，建议关注"
                        )
                    elif md_signal == "NEUTRAL" and signal_str in ("BUY", "SELL"):
                        all_evidence.append(
                            f"[多维度警告] 五维度评分中性，{signal_str}信号可能不可靠，建议降低仓位或观望"
                        )
                        # 降低置信度
        ...
        if self.data_store:
            try:
                ...
                feedback = ScoringFeedback(
                    ...
                    score_direction=score_direction,
                    confidence=confidence,
                    dimension_scores={dim_name: dim["score"] for dim_name, dim in dimension_scores.items()},
                    market_state=market_state,
                    trend_phase=phase,
