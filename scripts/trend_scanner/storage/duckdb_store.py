...
class DuckDBStore:
    ...
    def save_klines(self, symbol: str, df: pd.DataFrame, timeframe: str = "daily", source: str = "tqsdk"):
        """
        保存K线数据
    ...
    def get_klines(
        self, symbol: str, days: int = 120, timeframe: str = "daily", end_date: datetime = None
    ) -> pd.DataFrame | None:
        """
        获取K线数据
    ...
    def get_latest_kline(self, symbol: str, timeframe: str = "daily") -> dict[str, Any] | None:
        """
        获取最新K线数据
    ...
    def get_kline_date_range(self, symbol: str, timeframe: str = "daily") -> dict[str, Any] | None:
        """
        获取K线数据日期范围
        ...
        try:
            ...
            if result and result[0]:
                return {"earliest": result[0], "latest": result[1], "count": result[2]}
            return None
    ...
    def save_indicators(self, symbol: str, df: pd.DataFrame, indicator_names: list[str] = None):
        """
        保存技术指标数据
        ...
        if indicator_names is None:
            ...
            exclude_cols = {"date", "timestamp", "datetime", "open", "high", "low", "close", "volume", "open_interest"}
    ...
    def get_indicators(self, symbol: str, indicator_names: list[str] = None, days: int = 120) -> pd.DataFrame | None:
        """
        获取技术指标数据
        ...
        try:
            ...
            if indicator_names:
                ...
                """,
                    [symbol, start_date] + indicator_names,
                ).fetchdf()
            else:
                df = conn.execute(
                    """
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            stats["indicators"] = {"symbols": result[0], "indicator_types": result[1], "records": result[2]}
