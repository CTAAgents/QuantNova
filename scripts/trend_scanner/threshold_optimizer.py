...
class ThresholdOptimizer:
    ...
    def optimize_threshold(self, symbol: str = None, volatility_regime: str = None) -> dict:
        """
        基于历史反馈优化阈值
        ...
        # 计算基准（使用默认阈值）
        baseline_accuracy, baseline_coverage = self._evaluate_threshold(feedbacks, self.DEFAULT_BULLISH_THRESHOLD)
    ...
    def _evaluate_threshold(self, feedbacks: list[dict], threshold: float) -> tuple[float, float]:
        """
        评估阈值效果
    ...
    def optimize_for_regime(self, symbol: str = None) -> dict[str, dict]:
        ...
        for regime in ["high", "normal", "low"]:
            results[regime] = self.optimize_threshold(symbol=symbol, volatility_regime=regime)
    ...
    def get_threshold_report(self, symbol: str = None) -> str:
        ...
        if overall.get("insufficient_data"):
            ...
            if overall["improvement"] > 0.1:
                report.append(
                    f"- 建议将阈值从 {overall['baseline_threshold']:.4f} 调整为 {overall['optimal_threshold']:.4f}"
                )
            else:
