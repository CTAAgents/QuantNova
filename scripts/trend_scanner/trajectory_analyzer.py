...
class TrajectoryAnalyzer:
    ...
    def _create_avoidance_rule(self, pattern: Pattern) -> Optional[OptimizationRule]:
        """
        创建避免规则
    ...
    def _create_enhancement_rule(self, pattern: Pattern) -> Optional[OptimizationRule]:
        """
        创建增强规则
    ...
    def _generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            'summary': {
                ...
                'avg_pnl': sum(t.pnl_percent for t in self.trade_history) / len(self.trade_history) if self.trade_history else 0,
                'avg_success_pnl': sum(t.pnl_percent for t in self.success_cases) / len(self.success_cases) if self.success_cases else 0,
                'avg_failure_pnl': sum(t.pnl_percent for t in self.failure_cases) / len(self.failure_cases) if self.failure_cases else 0
            },
...
class FailureLearner:
    ...
    def _create_rule_from_failure_reason(self, reason: str, trades: List[TradeRecord]) -> Optional[OptimizationRule]:
        """
        从失败原因创建规则
