...
def calculate_vgrsi(
    ...
    result = pd.DataFrame({"vgrsi": vgrsi_values, "signal": signals}, index=prices.index)

    return result


def calculate_vgrsi_multi_timeframe(
    ...
    calculator = MultiTimeframeVGRSI(
        timeframe_configs=timeframe_configs, threshold_upper=threshold_upper, threshold_lower=threshold_lower
    )
    ...
    # 生成共识信号
    consensus_signals = calculator.generate_consensus_signals({tf: prices.values for tf, prices in prices_dict.items()})
...
class MultiTimeframeVGRSIFactor:
    ...
    def __init__(
        ...
        logger.info(f"MultiTimeframeVGRSIFactor 初始化完成，时间框架: {list(timeframe_configs.keys())}")
...
def consensus_factor(
    ...
    result = pd.DataFrame({"consensus": consensus_values, "signal": signals}, index=consensus_values.index)
