...
from scripts.trend_scanner.conceptual_feedback import BeliefManager, ConceptualFeedbackGenerator, TradeResult
from scripts.trend_scanner.factor_generator import FactorGenerator
from scripts.trend_scanner.report_parser import ReportAnalysis, ReportParser, ValidationPipeline
...
def sample_trade_records():
    ...
    return [
        ...
        },
        {
            ...
            "entry_price": 8200.0,
            "exit_price": 8350.0,
            "entry_time": "2026-06-04T09:00:00",
            "exit_time": "2026-06-05T15:00:00",
            "pnl": -150.0,
            "pnl_percent": -1.83,
            "holding_period": 1,
            "market_state": "volatile",
            "trend_phase": "EXHAUSTING",
            "volatility": "high",
            "er": 0.3,
            "tsi": -0.2,
            "rsi": 65.0,
        ...
        },
        {
            ...
            "entry_price": 14800.0,
            "exit_price": 14500.0,
            "entry_time": "2026-06-06T09:00:00",
...
def sample_report_metadata():
    """示例研报元数据"""
    return {"source": "test_research", "category": "supply_analysis", "published_at": "2026-06-15"}
...
class TestPhase4ConceptualFeedback:
    ...
    def test_belief_manager_create_and_query(self):
        ...
        belief = manager.create_belief(category="entry", content="高 ER 值时入场胜率更高", confidence=0.7)
...
class TestFullPipeline:
    ...
    def test_trajectory_to_feedback_to_belief(self, sample_trade_records, sample_trade_result):
        ...
        for lesson in feedback.key_lessons:
            belief = belief_mgr.create_belief(category="entry", content=lesson, confidence=feedback.confidence)
            assert belief.belief_id != ""
    ...
    def test_rl_design_with_trajectory_context(self, sample_trade_records):
        ...
        market_context = (
            f"基于 {report['summary']['total_trades']} 笔交易分析，胜率 {report['summary']['win_rate']:.1%}"
        )
        ...
        refinement = designer.refine_interface(
            ...
            expected_metrics={"sharpe": 1.0, "max_drawdown": 0.08, "win_rate": 0.6},
        )

        assert refinement["summary"]["total_diagnostics"] > 0

    def test_full_end_to_end_pipeline(
        self, sample_report_content, sample_report_metadata, sample_trade_records, sample_trade_result
    ):
        ...
        for vp in analysis.key_viewpoints[:3]:
            belief = belief_mgr.create_belief(
                category="market", content=vp.content if hasattr(vp, "content") else str(vp), confidence=0.6
            )
            assert belief.belief_id != ""
        ...
        # === 汇总验证 ===
        all_beliefs = belief_mgr.get_beliefs_by_category("market") + belief_mgr.get_beliefs_by_category("entry")
