...
class TestEndToEndPipeline(unittest.TestCase):
    ...
    def test_10_full_pipeline_simulation(self):
        ...
        recommended_route = next(
            r for r in brief['routes'] 
            if r['route_id'] == brief['recommended_route']
        )
        
        # 5. 模拟 Debater 修正
        if recommended_route['confidence'] < 0.7:
            debate_result = {
                'revised_confidence': recommended_route['confidence'] * 0.9,
                'divergence': 0.3
            }
        else:
            debate_result = {
                'revised_confidence': recommended_route['confidence'],
                'divergence': 0.1
            }
