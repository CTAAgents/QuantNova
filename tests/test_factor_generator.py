...
from trend_scanner.factor_generator import FactorGenerator, FactorKnowledgeManager, FactorResult, FactorValidator
...
class TestFactorGenerator:
    ...
    def setup_method(self):
        ...
        self.generator = FactorGenerator(
            llm_client=self.mock_llm_client, validator=self.validator, knowledge_manager=self.knowledge_manager
        )
...
"""
    ...
    def test_calculate_performance_metrics(self):
        ...
        df = pd.DataFrame({"close": prices, "volume": np.random.randint(1000, 10000, 100)}, index=dates)
    ...
    def test_get_factors_by_regime(self):
        ...
        for factor in self.manager.knowledge_base["factors"]:
            if factor["id"] == factor_id:
                factor["regime_effectiveness"] = {"trending": 0.9, "ranging": 0.3, "volatile": 0.6}
                break
