...
class TestBeliefManager:
    ...
    def test_create_belief(self):
        """测试创建信念"""
        belief = self.manager.create_belief(category="entry", content="趋势确认后入场", confidence=0.8)
    ...
    def test_update_belief(self):
        ...
        update = self.manager.update_belief(
            belief_id="belief_001", new_confidence=0.9, reason="成功交易验证", evidence="交易 trade_001 盈利 3.33%"
        )
    ...
    def test_get_belief(self):
        """测试获取信念"""
        # 创建信念
        self.manager.create_belief(category="entry", content="趋势确认后入场", confidence=0.8)
