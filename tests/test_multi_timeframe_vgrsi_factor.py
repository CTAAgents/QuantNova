...
from scripts.trend_scanner.visibility_graph import MultiTimeframeVGRSIFactor, consensus_factor
...
class TestMultiTimeframeVGRSIFactor:
    ...
    def test_custom_config(self):
        ...
        factor = MultiTimeframeVGRSIFactor(timeframe_configs=configs, threshold_upper=80.0, threshold_lower=20.0)
...
class TestEdgeCases:
    ...
    def test_single_timeframe(self):
        ...
        prices_dict = {"M1": pd.Series(np.cumsum(np.random.randn(100) * 0.1) + 100, index=dates)}
    ...
    def test_constant_prices(self):
        ...
        # 创建测试数据
        dates = pd.date_range("2024-01-01", periods=100, freq="D")
        prices_dict = {"M1": pd.Series([100.0] * 100, index=dates)}
