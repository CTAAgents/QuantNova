...
class TestReportParser:
    ...
    def test_generate_factor_suggestions(self):
        ...
        suggestions = self.parser._generate_factor_suggestions(viewpoints, data_logic, logic_chains)
...
class TestValidationPipeline:
    ...
    def test_validate_analysis_with_data(self):
        ...
        analysis = ReportAnalysis(
            ...
            data_logic=[DataLogic(metric="库存", value="100万吨", context="库存处于低位", implication="影响供需平衡")],
            logic_chains=[
                LogicChain(cause="供应收紧", effect="价格上涨", mechanism="供应收紧 → 价格上涨", confidence=0.7)
            ],
            factor_suggestions=[
                FactorSuggestion(
    ...
    def test_calculate_quality_score(self):
        ...
        analysis = ReportAnalysis(
            ...
            key_viewpoints=[
                KeyViewpoint(
                    viewpoint_id="vp_001", content="观点1", category="supply", confidence=0.8, supporting_data=[]
                ),
                KeyViewpoint(
                    viewpoint_id="vp_002", content="观点2", category="demand", confidence=0.7, supporting_data=[]
                ),
            ],
            data_logic=[DataLogic(metric="库存", value="100万吨", context="库存低", implication="影响供需")],
            logic_chains=[LogicChain(cause="供应", effect="价格", mechanism="供应→价格", confidence=0.7)],
            factor_suggestions=[
                FactorSuggestion(
                    suggestion_id="s1",
                    name="因子1",
...
class TestDataLogic:
    ...
    def test_create_data_logic(self):
        ...
        data_logic = DataLogic(
            metric="焦煤库存", value="100万吨", context="库存处于历史低位", implication="影响供需平衡"
        )
...
class TestLogicChain:
    ...
    def test_create_logic_chain(self):
        ...
        logic_chain = LogicChain(
            cause="安全检查限产", effect="供应收紧", mechanism="安全检查限产 → 供应收紧", confidence=0.7
        )
