...
class TestStateSpaceDesigner:
    ...
    def test_design_state_space_with_rules(self):
        ...
        design = self.designer.design_state_space(market_context, trading_objective, available_data)
...
class TestRewardFunctionDesigner:
    ...
    def test_design_reward_function_with_rules(self):
        ...
        design = self.designer.design_reward_function(trading_objective, risk_rules, market_context)
...
class TestDiagnosticRefiner:
    ...
    def test_collect_diagnostics(self):
        """测试收集诊断结果"""
        training_metrics = {"reward_mean": 0.5, "episode_length": 100, "loss": 0.1}
        expected_metrics = {"reward_mean": [0.3, 0.7], "episode_length": [50, 150], "loss": [0.01, 0.2]}

        diagnostics = self.refiner.collect_diagnostics(training_metrics, expected_metrics)
    ...
    def test_collect_diagnostics_with_warning(self):
        ...
        expected_metrics = {"reward_mean": [0.3, 0.7], "episode_length": [50, 150]}
...
class TestRLInterfaceDesigner:
    ...
    def test_design_interface(self):
        ...
        interface = self.designer.design_interface(market_context, trading_objective, available_data, risk_rules)
    ...
    def test_refine_interface(self):
        """测试修正 RL 接口"""
        # 当前设计
        current_design = {"state_space": {"features": []}, "reward_function": {"components": []}}

        # 训练指标
        training_metrics = {"reward_mean": 0.2, "episode_length": 100}

        # 预期指标
        expected_metrics = {"reward_mean": [0.3, 0.7], "episode_length": [50, 150]}

        refinement = self.designer.refine_interface(current_design, training_metrics, expected_metrics)
