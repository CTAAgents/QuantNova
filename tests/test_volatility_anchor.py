...
from scripts.trend_scanner.volatility_anchor import VolatilityAnchor, volatility_anchor
...
class TestVolatilityAnchor:
    ...
    def test_calculate_basic(self):
        ...
        df = pd.DataFrame({"high": base + 1, "low": base - 1})
...
class TestConvenienceFunction:
    ...
    def test_volatility_anchor_basic(self):
        ...
        n = 100
        df = pd.DataFrame(
            {"high": np.cumsum(np.random.randn(n) * 0.5) + 100, "low": np.cumsum(np.random.randn(n) * 0.5) + 98}
        )
...
class TestEdgeCases:
    ...
    def test_single_row(self):
        ...
        df = pd.DataFrame({"high": [100.0], "low": [98.0]})
        result = anchor.calculate(df)
    ...
    def test_constant_prices(self):
        ...
        df = pd.DataFrame({"high": [100.0] * 20, "low": [98.0] * 20})
        result = anchor.calculate(df)
    ...
    def test_high_volatility(self):
        ...
        df = pd.DataFrame({"high": base + 5, "low": base - 5})
