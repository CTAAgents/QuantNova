...
class TestWalkForwardValidator:
    ...
    def test_check_pass_criteria(self):
        ...
        # 通过的情况
        is_metrics = {"sharpe": 1.0, "win_rate": 0.6, "max_drawdown": 0.1, "trades": 10}
        oos_metrics = {"sharpe": 0.8, "win_rate": 0.5, "max_drawdown": 0.15, "trades": 8}
        assert validator._check_pass_criteria(is_metrics, oos_metrics) == True

        # 不通过的情况（OOS Sharpe 太低）
        oos_metrics_low_sharpe = {"sharpe": 0.2, "win_rate": 0.5, "max_drawdown": 0.15, "trades": 8}
        assert validator._check_pass_criteria(is_metrics, oos_metrics_low_sharpe) == False

        # 不通过的情况（OOS 回撤太大）
        oos_metrics_high_drawdown = {"sharpe": 0.8, "win_rate": 0.5, "max_drawdown": 0.3, "trades": 8}
        assert validator._check_pass_criteria(is_metrics, oos_metrics_high_drawdown) == False

        # 不通过的情况（交易次数太少）
        oos_metrics_few_trades = {"sharpe": 0.8, "win_rate": 0.5, "max_drawdown": 0.15, "trades": 2}
