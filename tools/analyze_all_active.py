...
class ActiveSymbolAnalyzer:
    ...
    def step2_filter_active(self, symbols: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for s in active_symbols[:20]:  # 只显示前20个
            print(f"{s['symbol']:<10} {s['exchange']:<8} {s['last_price']:<12.2f} "
    ...
    def step3_analyze_symbols(self, active_symbols: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, symbol_info in enumerate(active_symbols, 1):
            ...
            try:
                ...
                print(f"  [结果] 方向={direction}, 置信度={confidence:.2f}, "
                      f"趋势强度={trend_strength:.2f}, ER={er:.2f}")
    ...
    def generate_report(self, results: List[Dict[str, Any]]) -> str:
        ...
        if short_symbols:
            ...
            for r in sorted(short_symbols, key=lambda x: x.get('indicators', {}).get('trend_strength', 0), reverse=True):
                self._append_symbol_report(report_lines, r)
        ...
        if failed:
            ...
            for r in failed:
                report_lines.append(f"\n{r['symbol']} ({r.get('exchange', 'N/A')}): "
                                   f"{r.get('reason', r.get('error', '未知原因'))}")
