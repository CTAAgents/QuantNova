...
def compute_all_indicators(df: pd.DataFrame, fast: bool = False) -> dict[str, pd.Series]:
    ...
    indicators["trma"] = (
        df["close"]
        .rolling(20)
        .apply(lambda x: np.mean(np.arange(1, len(x) + 1) * x / np.sum(np.arange(1, len(x) + 1))), raw=True)
    )
    ...
    indicators["vr"] = (
        (up_vol.rolling(vr_period).sum() + eq_vol.rolling(vr_period).sum() / 2)
        / (down_vol.rolling(vr_period).sum() + eq_vol.rolling(vr_period).sum() / 2)
        * 100
    )
    ...
    indicators["br"] = (
        (df["high"] - df["close"].shift(1)).rolling(arbr_period).sum()
        / (df["close"].shift(1) - df["low"]).rolling(arbr_period).sum()
        * 100
    )
    ...
    wvad_period = 24
    indicators["wvad"] = (
        ((df["close"] - df["open"]) / (df["high"] - df["low"]) * df["volume"]).rolling(wvad_period).sum()
    )
    ...
    for i in range(1, len(df)):
        if df["close"].iloc[i - 1] != 0:
            pvt.iloc[i] = (
                pvt.iloc[i - 1]
                + df["volume"].iloc[i] * (df["close"].iloc[i] - df["close"].iloc[i - 1]) / df["close"].iloc[i - 1]
            )
        else:
    ...
    dkx_wma = dkx_mid.rolling(20).apply(
        lambda x: np.sum(x * dkx_weights[: len(x)]) / np.sum(dkx_weights[: len(x)]) if len(x) == 20 else np.nan,
        raw=True,
    )
    ...
    lon_period = 10
    lon_var1 = (
        (df["close"] - df["low"].rolling(lon_period).min())
        / (df["high"].rolling(lon_period).max() - df["low"].rolling(lon_period).min())
        * 100
    )
    ...
    indicators["priceosc"] = (
        (df["close"].rolling(12).mean() - df["close"].rolling(26).mean()) / df["close"].rolling(12).mean() * 100
    )
    ...
    for symbol in target_symbols:
        try:
            ...
            if result["status"] == "success":
                print(
                    f"  {symbol}: {result['rows']} 条记录, {result['indicators']} 个指标, 最新: {result['latest_date']}"
                )
                success += 1
