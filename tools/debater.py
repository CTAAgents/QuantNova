...
class DebaterAgent:
    ...
    def __init__(self, config: dict[str, Any] = None):
        ...
        self.debate_engine = DebateReasoningEngine(
            hawk_llm=WorkBuddyAgentProvider(model="default"), dove_llm=WorkBuddyAgentProvider(model="default")
        )
    ...
    def debate(self, brief: dict[str, Any], force: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            snapshot = IndicatorSnapshot(
                timestamp=brief.get("timestamp", datetime.now().isoformat()), close=0, high=0, low=0, open=0, volume=0
            )
            ...
            try:
                ...
                if hawk_args or dove_args:
                    feedback = feedback_gen.generate_feedback(
                        symbol=symbol, hawk_perspective=hawk_args, dove_perspective=dove_args, market_context=context
                    )
            ...
            try:
                ...
                bp_result = bp_manager.propagate_beliefs(
                    symbol=symbol, debate_result=debate_result, market_context=context
                )
