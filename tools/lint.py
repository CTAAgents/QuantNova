...
def main():
    ...
    if "--format" in args:
        # 仅格式化
        exit_code = run_command(["ruff", "format"] + existing_dirs, "格式化代码")
    elif "--fix" in args:
        # 检查 + 修复
        exit_code = run_command(["ruff", "check", "--fix"] + existing_dirs, "检查并修复代码风格")
        # 格式化
        format_code = run_command(["ruff", "format"] + existing_dirs, "格式化代码")
        if format_code != 0:
            exit_code = format_code
    elif "--check-only" in args:
        # 仅检查
        exit_code = run_command(["ruff", "check"] + existing_dirs, "检查代码风格")
    else:
        # 默认：检查 + 显示建议
        exit_code = run_command(["ruff", "check", "--show-fixes"] + existing_dirs, "检查代码风格（显示可修复项）")

        # 同时检查格式
        format_check = run_command(["ruff", "format", "--check"] + existing_dirs, "检查代码格式")
        if format_check != 0:
