...
EVOLUTION_STATE_FILE = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "evolution_state.json"
)
...
def load_evolution_state() -> dict[str, Any]:
    ...
    with open(EVOLUTION_STATE_FILE, encoding="utf-8") as f:
        return json.load(f)
...
def run_evolution(trigger_reason: str = "手动触发", memory_bridge=None):
    ...
    if loss_trades:
        # 分析亏损交易的共同特征
        early_entry_count = sum(
            1 for t in loss_trades if "入场过早" in t.get("notes", "") or "趋势未确立" in t.get("notes", "")
        )
        tight_stop_count = sum(
            1 for t in loss_trades if "止损过紧" in t.get("notes", "") or "震出局" in t.get("notes", "")
        )
    ...
    report_path = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "latest_evolution.json"
    )
...
def show_status():
    ...
    print("\n自进化触发条件:")
    print(
        f"  连续亏损 >= {evolver_config.get('consecutive_losses', 3)} 次 (当前: {state.get('consecutive_losses', 0)})"
    )
    print(
        f"  累计亏损 >= {evolver_config.get('cumulative_loss_pct', 10)}% (当前: {abs(state.get('cumulative_pnl_pct', 0)):.1f}%)"
    )
