...
def scan_symbol(
    ...
    try:
                            ...
                            try:
                                ...
                                if factor_func:
                                    ...
                                    if factor_values is not None and len(factor_values) > 0:
                                        latest_factor_value = (
                                            float(factor_values.iloc[-1])
                                            if hasattr(factor_values, "iloc")
                                            else float(factor_values[-1])
                                        )
        ...
        try:
            ...
            position_info = sizer.calculate(trend_strength=trend_strength, volatility=volatility)
        except Exception as e:
        ...
        try:
            ...
            atr = float(latest.get("atr", 0))
            if atr > 0:
                stop_price = stop_calc.atr_stop(entry_price=float(latest.get("close", 0)), atr=atr, direction=direction)
                stop_loss_info = {
...
def scan_all(
    ...
    try:
        ...
        print(
            f"宏观状态: {macro_state['cycle']['name']} | {macro_state['liquidity']['name']} | {macro_state['risk_appetite']['name']}"
        )
        print(
            f"  策略权重: 趋势={macro_state['strategy_weights']['trend_following']:.0%} 均值回归={macro_state['strategy_weights']['mean_reversion']:.0%}"
        )
    except Exception as e:
        print(f"宏观状态检测跳过: {e}")
    ...
    # 创建扫描结果
    scan_result = create_scan_result(total_scanned=len(symbols), signals=signals, no_signal_symbols=no_signal_symbols)
...
def main():
    ...
    parser.add_argument("--evaluate-factors", action="store_true", help="扫描后运行截面 IC/ICIR 因子评估")
    parser.add_argument("--evolve", action="store_true", help="运行闭环因子进化引擎")
    parser.add_argument("--optimize-params", action="store_true", help="运行贝叶斯参数优化")
    parser.add_argument("--evolve-rounds", type=int, default=5, help="进化轮数（默认 5）")
    parser.add_argument("--evolve-target", type=int, default=3, help="目标晋升因子数（默认 3）")
    parser.add_argument("--opt-trials", type=int, default=30, help="参数优化试验次数（默认 30）")
    parser.add_argument("--load-report", type=str, default=None, help="从研报文件加载种子因子（配合 --evolve 使用）")
    parser.add_argument("--health-check", action="store_true", help="运行策略健康度评估")
    parser.add_argument("--position-health", action="store_true", help="运行持仓健康度评估（基于技术指标+LLM推理）")
    parser.add_argument("--overfitting-check", action="store_true", help="运行过拟合检测")
    parser.add_argument("--execution-check", action="store_true", help="运行执行引擎风控检查")
    parser.add_argument("--reasoner", action="store_true", help="启用Reasoner Agent深度分析（输出决策简报）")
    parser.add_argument(
        "--use-multi-dimension",
        action="store_true",
    ...
    if args.evaluate_factors:
        ...
        print(f"{'=' * 60}")
        try:
            from trend_scanner.factor_evaluator import BUILTIN_FACTORS, FactorEvaluator
        ...
        try:
            ...
            if args.load_report:
                ...
                if os.path.exists(args.load_report):
                    with open(args.load_report, encoding="utf-8") as f:
                        report_content = f.read()
                    n = engine.load_seeds_from_report(report_content, {"title": os.path.basename(args.load_report)})
