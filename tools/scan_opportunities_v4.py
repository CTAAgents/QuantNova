...
# 导入因子生成器模块
from trend_scanner.factor_generator import FactorGenerator, FactorKnowledgeManager, FactorValidator
from trend_scanner.indicators import IndicatorEngine
from trend_scanner.llm_factor_client import MockLLMClient
...
def scan_all(symbols: list[str] = None, use_factors: bool = False) -> dict[str, Any]:
    ...
    if use_factors:
        try:
            ...
            factor_generator = FactorGenerator(
                llm_client=llm_client, validator=validator, knowledge_manager=knowledge_manager
            )
    ...
    for symbol in symbols:
        result = scan_symbol(
            symbol, data_source, signal_filter, factor_generator=factor_generator, use_factors=use_factors
        )
        if result:
