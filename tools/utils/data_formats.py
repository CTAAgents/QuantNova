...
def create_scan_result(
    total_scanned: int, signals: list[dict[str, Any]], no_signal_symbols: list[str] = None
) -> dict[str, Any]:
    """
    创建扫描结果数据结构
...
def read_scan_result() -> dict[str, Any] | None:
    """读取最新扫描结果"""
    if not os.path.exists(LATEST_SCAN_FILE):
        return None
    with open(LATEST_SCAN_FILE, encoding="utf-8") as f:
        return json.load(f)
...
def create_monitor_result(
    positions_monitored: int, alerts: list[dict[str, Any]], no_alert_positions: list[str] = None
) -> dict[str, Any]:
    """
    创建监控结果数据结构
...
def create_alert(
    symbol: str, alert_type: str, severity: str, indicators: dict[str, Any], trigger_reason: str
) -> dict[str, Any]:
    """
    创建单个预警数据结构
...
def read_monitor_result() -> dict[str, Any] | None:
    """读取最新监控结果"""
    if not os.path.exists(LATEST_MONITOR_FILE):
        return None
    with open(LATEST_MONITOR_FILE, encoding="utf-8") as f:
        return json.load(f)
...
def get_signal_filter() -> dict[str, Any]:
    ...
    return scanner_config.get(
        "signal_filter", {"er_min": 0.6, "tsi_min": 20, "tsi_max": -20, "trend_strength_min": 0.5, "r2_min": 0.4}
    )
